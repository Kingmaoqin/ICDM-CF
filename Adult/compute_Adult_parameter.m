...
filePath = 'E:\CF\PlatEMO\PlatEMO\Data\MaOEADPP123/MaOEADPP123_Adult_CFEGenerator_M3_D108_15.mat';
dAta = load(filePath);
...
Original_Data = readtable('Adultdata.csv');    % Adult数据集
Original_Data = table2array(Original_Data);
Original_Data = Original_Data(2:end,2:end);
Original_X = Original_Data(1,[1:6, 8:end]);  % 读第1行的原始数据
Original_X = Original_X';
Original_Y = Original_Data(1,7);
% 找出需要删除的行索引
Delete = abs(Original_Y-predicted_Y)<=0.5;
Data_XKept = Data_X; % 保留不需要删除的行
