...
filePath = 'E:\CF\PlatEMO\PlatEMO\Data\MaOEADPP123/MaOEADPP123_COMPAS_CFEGenerator_M3_D11_1.mat';
dAta = load(filePath);
% 加载模型
...
Original_Data = readtable('COMPASdata.csv');
Original_Data = table2array(Original_Data);
Original_Data = Original_Data(2:end,:);
Original_X = Original_Data(5,1:end-1);  % 读第5行的原始数据
Original_X = Original_X';
Original_Y = Original_Data(5,end);
% 找出需要删除的行索引
Delete = abs(Original_Y-predicted_Y)<=0.5;
Data_XKept = Data_X(~Delete, :); % 保留不需要删除的行
Data_YKept = predicted_Y(~Delete, :);
continuous_indices = 1;
categorical_indices = setdiff(1:11, continuous_indices); % 假设除了连续特征外的都是分类特征

data = Original_Data;
compute_pre_MAD = data(:,1:end-1);
